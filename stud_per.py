...
def preprocessing_input_data(data,scaler ,le):
    data['Extracurricular Activities(x3)'] = le.transform([data['Extracurricular Activities(x3)']])[0]
...
def main():
    ...
    if st.button("predict_your_score"):
        user_data = {
            "Hours Studied(x1)":hour_studied,
            "Previous Scores(x2)":Previous_Scores,
            "Extracurricular Activities(x3)":Extracurricular_Activities,
            "Sleep Hours(x4)":Sleep_Hours,
            "Sample Question Papers Practiced(x4)":Question_Papers
